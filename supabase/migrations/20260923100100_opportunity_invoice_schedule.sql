-- Facturation échelonnée : table invoice_schedule, RLS, trigger somme=price,
-- et branche echellonne dans load_analyses_payload.
...
CREATE TABLE public.invoice_schedule (
  ...
  CONSTRAINT invoice_schedule_amount_positive CHECK (amount > 0)
);

CREATE INDEX idx_invoice_schedule_opportunity_id
  ON public.invoice_schedule (opportunity_id);
...
CREATE POLICY "invoice_schedule_select_active" ON public.invoice_schedule
  FOR SELECT USING (
...
CREATE POLICY "invoice_schedule_insert_active" ON public.invoice_schedule
  FOR INSERT WITH CHECK (public.can_access_opportunity(opportunity_id));

CREATE POLICY "invoice_schedule_update_active" ON public.invoice_schedule
  FOR UPDATE USING (public.can_access_opportunity(opportunity_id))
  WITH CHECK (public.can_access_opportunity(opportunity_id));

CREATE POLICY "invoice_schedule_delete_active" ON public.invoice_schedule
  FOR DELETE USING (public.can_access_opportunity(opportunity_id));

-- Cohérence : si fréquence = echellonne, somme(amount) = price (et >= 2 échelons).
-- Déferrable pour permettre delete+insert dans la même transaction (RPC sync).
CREATE OR REPLACE FUNCTION public.check_invoice_schedule_sum()
RETURNS trigger
LANGUAGE plpgsql
...
DECLARE
  v_opp_id uuid;
  v_freq public.opportunity_invoice_frequency_enum;
  v_price numeric;
  v_sum numeric;
  v_count integer;
BEGIN
  IF TG_TABLE_NAME = 'opportunity' THEN
    ...
    END IF;
    v_opp_id := NEW.id;
    v_freq := NEW.invoice_frequency;
    v_price := NEW.price;
  ELSE
    v_opp_id := COALESCE(NEW.opportunity_id, OLD.opportunity_id);
    SELECT o.invoice_frequency, o.price
    INTO v_freq, v_price
    FROM public.opportunity o
    WHERE o.id = v_opp_id;
  END IF;

  IF v_freq IS DISTINCT FROM 'echellonne' THEN
    RETURN COALESCE(NEW, OLD);
  END IF;

  IF v_price IS NULL THEN
    RAISE EXCEPTION
      'Montant opportunité requis pour une facturation échelonnée.';
  END IF;

  SELECT coalesce(sum(s.amount), 0), count(*)::integer
  INTO v_sum, v_count
  FROM public.invoice_schedule s
  WHERE s.opportunity_id = v_opp_id;
  ...
  IF v_sum <> v_price THEN
    RAISE EXCEPTION
      'La somme des échelons (%) doit égaler le montant de l''opportunité (%).',
      v_sum, v_price;
  END IF;
...
CREATE CONSTRAINT TRIGGER trg_invoice_schedule_sum_on_schedule
  AFTER INSERT OR UPDATE OR DELETE ON public.invoice_schedule
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW
  EXECUTE FUNCTION public.check_invoice_schedule_sum();

CREATE CONSTRAINT TRIGGER trg_invoice_schedule_sum_on_opportunity
  AFTER UPDATE OF price, invoice_frequency ON public.opportunity
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW
  EXECUTE FUNCTION public.check_invoice_schedule_sum();

-- Remplace l'échéancier et aligne price / fréquence / end_at dans une seule transaction
-- (nécessaire car le client Supabase ne partage pas de transaction entre appels).
CREATE OR REPLACE FUNCTION public.sync_opportunity_echellonne_billing(
...
AS $$
BEGIN
  IF NOT public.can_access_opportunity(p_opportunity_id) THEN
    RAISE EXCEPTION 'Accès refusé à l''opportunité.';
  END IF;

  IF p_price IS NULL THEN
    RAISE EXCEPTION 'Montant opportunité requis pour une facturation échelonnée.';
  END IF;

  IF p_rows IS NULL
     OR jsonb_typeof(p_rows) <> 'array'
     OR jsonb_array_length(p_rows) < 2
     OR jsonb_array_length(p_rows) > 12 THEN
    RAISE EXCEPTION 'Une facturation échelonnée requiert entre 2 et 12 échéances.';
  END IF;

  UPDATE public.opportunity
  SET
    price = p_price,
    invoice_frequency = 'echellonne',
    end_at = NULL
  ...
  DELETE FROM public.invoice_schedule
  WHERE opportunity_id = p_opportunity_id;

  INSERT INTO public.invoice_schedule (opportunity_id, invoice_at, amount)
  SELECT
    p_opportunity_id,
    (elem->>'invoice_at')::date,
    (elem->>'amount')::numeric
  FROM jsonb_array_elements(p_rows) AS elem;
END;
$$;

REVOKE ALL ON FUNCTION public.sync_opportunity_echellonne_billing(uuid, numeric, jsonb)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.sync_opportunity_echellonne_billing(uuid, numeric, jsonb)
  TO authenticated;

-- Vide l'échéancier (après passage hors echellonne, ou nettoyage).
CREATE OR REPLACE FUNCTION public.clear_opportunity_invoice_schedule(
  p_opportunity_id uuid
)
RETURNS void
LANGUAGE plpgsql
...
REVOKE ALL ON FUNCTION public.clear_opportunity_invoice_schedule(uuid)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.clear_opportunity_invoice_schedule(uuid)
  TO authenticated;
