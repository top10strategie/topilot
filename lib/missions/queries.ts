...
export async function listMissionsPage(
  ...
  const { data, error } = await supabase.rpc("list_missions_page", {
    ...
    p_scope: filters.scope || undefined,
    p_statuses: filters.statuses.length > 0 ? filters.statuses : undefined,
    p_start_from: filters.startFrom || undefined,
    p_start_to: filters.startTo || undefined,
    p_end_from: filters.endFrom || undefined,
    p_end_to: filters.endTo || undefined,
