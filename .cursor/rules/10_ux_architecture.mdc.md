...
- **Opportunités** :
    ...
    - **Évolution du pipeline Commercial** : courbes mensuelles **CA engagé** / **CA prévisionnel** (répartition `invoice_frequency` + `end_at` ou `invoice_schedule` si `echellonne`, cf. `03_business_rules.mdc`) ; sélecteur d’année indépendant ; compteur des opportunités sans délais de facturation.
