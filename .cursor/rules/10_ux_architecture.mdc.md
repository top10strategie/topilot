...
- **Vue Kanban** — **vue par défaut** à l'ouverture de la page (cf. `07_ux_composants_reutilisable.mdc` sections 4.3 et 8).
    ...
    - Les colonnes **Gagné** et **Perdue** n'affichent que les opportunités dont `closed_at` (repli `due_date_at`) date de **moins d'un mois** (cf. `03_business_rules.mdc`) — les archives plus anciennes restent accessibles via « Inclure les archivées » ou un filtre de statut terminal. Ces deux colonnes sont triées par `due_date_at` décroissant (plus récentes en haut) ; les colonnes actives restent triées par `kanban_order`. La même fenêtre d'1 mois s'applique aux vues Cartes et Tableau.
...
- **Tiroir de création "Nouvelle opportunité"** (cf. section 7), en **deux temps** :
    1. **Bloc identification** (toujours visible en haut) : Titre (`opportunity_name`, obligatoire), Client (`client_id`, **obligatoire** — dropdown clients + bouton d'ajout client), Contact (`contact_client_id`, nullable — dropdown contacts du client sélectionné + bouton d'ajout contact), Responsable opportunité (`collaborator_id`, obligatoire — dropdown collaborateurs), Date de dernière rencontre (`last_meeting_at`), Échéance (`due_date_at`, **obligatoire**), Début de la facturation (`closed_at`, optionnel — même champ qu’en édition). Bouton **"Enregistrer"** dédié : crée l'opportunité en base avec ces champs — nécessaire pour permettre l'ajout de documents liés qui requièrent un `opportunity_id` existant. Le statut initial (`kanban_status`) et la probabilité associée sont posés automatiquement à cet instant selon l'historique du client (cf. `03_business_rules.mdc`), sans champ visible à cette étape.
    2. **Bloc complémentaire** (déverrouillé après l'étape 1 ; immédiatement visible en édition) :
        ...
        - Fréquence de facturation (`invoice_frequency`) : à gauche ; à droite **Fin de facturation** (`end_at`) si `unique|mensuel|trimestriel|annuel` ; section **Choix des échelons** en dessous si `echellonne` (2–12 lignes date + montant €, bouton `+`, récap total vs `price` ; confirmation avant purge si on quitte `echellonne`)
...
- **Informations** :
    ...
    - Colonne droite : Statut (`kanban_status`), Montant (`price`), Montant pondéré (`average_price`, **lecture seule**, calculé), Probabilité (`probability_confirmation`), Urgence (`priority`), Action (`action`), Source (`source`), Date de dernière rencontre (`last_meeting_at`), Échéance (`due_date_at`), Fréquence de facturation (`invoice_frequency`), puis **Fin de facturation** (`end_at`) ou liste des **échelons** (`invoice_schedule`) si `echellonne`.
...
- **Vue Kanban** (cf. `07_ux_composants_reutilisable.mdc` section 8) : 4 colonnes, une par valeur de `kanban_status`, dans l'ordre de déclaration de l'enum : **A faire → En cours → Terminée → Archivée**.
    - Les colonnes **Terminé** et **Archivé** n'affichent que les missions terminées depuis **moins d'un mois** (`completed_at` / `archived_at`, repli `end_at` — cf. `03_business_rules.mdc`/`04_database_schema.mdc`). La même fenêtre s'applique aux vues Cartes et Tableau ; les plus anciennes restent accessibles via le filtre par statut (hors board).
...
- **Opportunités** :
    - Bandeau : 2 KPI filtrés sur l’**année calendaire Paris courante**, titres suffixés ` - {année}` — pas de sélecteur d’année :
      - **Total des sommes engagées** : somme des échéances de facturation (`invoice_frequency` / `invoice_schedule`) des opportunités `gagne` dont le mois tombe en Jan–Déc de l’année ; ligne secondaire `nombre d'engagée non renseigné : {n}` (gagnées sans infos de facturation, `closed_at` année Paris ou NULL).
      - **Total des sommes pondérées** : opportunités hors `gagne`/`perdue` ; échéances × `probability_confirmation/100` (ou `average_price` entier sur le mois de `due_date_at` si pas d’infos de facturation) ; ligne secondaire `total prévisionnel : {SUM price brut même logique}`.
      - **Comparaison par statut** à droite (barres verticales, même filtre année d’attribution `closed_at`/`due_date_at`, même suffixe).
    - **Évolution du pipeline Commercial** : barres verticales **empilées** mensuelles **CA engagé** / **CA prévisionnel** (répartition `invoice_frequency` + `end_at` ou `invoice_schedule` si `echellonne`, cf. `03_business_rules.mdc`, montants `price` bruts) ; ligne **Objectif de CA** (`revenue_aim.amount / 12`) ; ligne **CA engagé N−1** (année sélectionnée − 1) ; sélecteur d’année ; compteur des opportunités sans informations de facturation.
