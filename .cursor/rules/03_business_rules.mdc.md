...
- `archived_at` (timestamptz, posé automatiquement par trigger dès que `kanban_status = archivee`, remis à `NULL` sinon) : les missions **terminées** (`terminee`, via `completed_at` ou repli `end_at`) et **archivées** (`archivee`, via `archived_at` ou repli `end_at`) depuis **plus d'un mois** sont masquées sur Kanban, Cartes et Tableau. Réaffichage via filtre de statut explicitement terminal (hors vue Kanban board).
