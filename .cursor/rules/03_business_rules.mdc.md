...
- Une opportunité peut avoir plusieurs **catégories** (`opportunity_category`), **documents liés** (`opportunity_document`), **outils liés** (`opportunity_tool`) et **wikis liés** (`opportunity_wiki`).
- Champ de texte libre : **`notes`**, couplé à `notes_updated_at`, historisé dans `audit_log`.
